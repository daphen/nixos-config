...
func TestApplyCoversEveryAdapterAndContinuesAfterFailures(t *testing.T) {
	...
	checks := map[string]string{
		...
		"home/.local/share/themes/Dotfiles-dark/gtk-4.0/gtk.css":       "@import url(\"resource:///org/gtk/libgtk/theme/Default/gtk-dark.css\");\ngtk theme\n",
...
func TestSwitchGTKUsesReloadableThemesAndClearsOverrides(t *testing.T) {
	...
	custom := f.write("home/.config/gtk-4.0/gtk.css", "/* My own CSS */")
	for _, mode := range []string{"dark", "light", "dark"} {
		...
		for _, version := range []string{"gtk-3.0", "gtk-4.0"} {
			...
			if !strings.HasPrefix(css, "@import url(\"resource:///org/gtk/libgtk/theme/") || !strings.HasSuffix(css, "gtk theme\n") {
				t.Errorf("%s %s CSS: %q", mode, version, css)
			}
			variant := mustRead(t, filepath.Join(f.home, ".local/share/themes", "Dotfiles-"+mode, version, "gtk-dark.css"))
			if variant != css {
				t.Errorf("%s %s dark variant differs from selected theme", mode, version)
			}
			settings := mustRead(t, filepath.Join(f.home, ".config", version, "settings.ini"))
			if !strings.Contains(settings, "gtk-theme-name=Dotfiles-"+mode+"\n") {
				t.Errorf("%s settings: %s", version, settings)
			}
		}
		if !strings.Contains(readIfExists(log), "dconf write /org/gnome/desktop/interface/gtk-theme 'Dotfiles-"+mode+"'") {
			t.Fatalf("GTK theme was not broadcast: %s", readIfExists(log))
		}
	}
	if _, err := os.Stat(legacy); !os.IsNotExist(err) {
		t.Fatalf("startup-only generated CSS was not removed: %v", err)
	}
	if got := mustRead(t, custom); got != "/* My own CSS */" {
	...
	if strings.Contains(commands, "set -Ux GTK_THEME") || strings.Contains(commands, "set-environment GTK_THEME=") {
		t.Fatalf("forced theme was exported: %s", commands)
	}
}
...
func TestToggleFallsBackWhenGSettingsSchemasAreMissing(t *testing.T) {
	f := setup(t)
