...
func (m *manager) applyGTK(generated, mode string) {
	dark, variant := "false", ""
	if mode == "dark" {
		dark, variant = "true", "-dark"
	}
	css, err := os.ReadFile(generated)
	if err != nil {
		m.failure("Could not read GTK theme: " + err.Error())
		return
	}
	name := "Dotfiles-" + mode
	...
	for _, version := range []string{"gtk-3.0", "gtk-4.0"} {
		...
		themeDir := filepath.Join(m.home, ".local/share/themes", name, version)
		bestEffort(os.MkdirAll(themeDir, 0755))
		stylesheet := fmt.Sprintf("@import url(\"resource:///org/gtk/libgtk/theme/%s/gtk%s.css\");\n%s", base, variant, css)
		for _, file := range []string{"gtk.css", "gtk-dark.css"} {
			bestEffort(os.WriteFile(filepath.Join(themeDir, file), []byte(stylesheet), 0644))
		}
		dir := filepath.Join(m.home, ".config", version)
		bestEffort(os.MkdirAll(dir, 0755))
		legacy := filepath.Join(dir, "gtk.css")
		previous, _ := os.ReadFile(legacy)
		// GTK caches user CSS at startup; only named themes reload on a theme change.
		if strings.HasPrefix(string(previous), "/* GTK theme overrides - auto-generated by theme-manager") {
			bestEffort(os.Remove(legacy))
		}
...
func (m *manager) validateWallpaper(mode string) {
