...
local function gather()
	...
	return {
		...
		add = shared_diff and shared_diff.authoritative and (shared_diff.add or 0) or (shared_diff and shared_diff.add or diff_cache.add),
		del = shared_diff and shared_diff.authoritative and (shared_diff.del or 0) or (shared_diff and shared_diff.del or diff_cache.del),
		diffState = shared_diff and (shared_diff.loading and "loading" or (shared_diff.error and "unavailable" or "ready")) or "local",
		plan = vim.trim(plan),
		root = root_chip(),
		dashboard = dashboard,
