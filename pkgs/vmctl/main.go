...
type mutagenListing struct {
	...
	Alpha, Beta struct{ Protocol, User, Host, Path string }
	Ignore      struct {
...
func (s syncRun) managedSession() (bool, error) {
	text, err := s.a.combined(s.mutagen, "sync", "list", "--template", "{{ json . }}", s.name)
	if err != nil {
		return false, nil
	}
	...
	m := sessions[0]
	if m.Name != s.name || m.Alpha.Protocol != "ssh" || m.Alpha.User != s.a.user || m.Alpha.Host != s.a.host || filepath.Clean(m.Alpha.Path) != s.vmwt || m.Beta.Protocol != "local" || filepath.Clean(m.Beta.Path) != s.local {
		return false, fmt.Errorf("Mutagen session %s has mismatched endpoints; refusing to modify it", s.name)
	}
	for _, pattern := range m.Ignore.Paths {
		if pattern == "bazel-*" {
			return false, fmt.Errorf("Mutagen session %s has unsafe unanchored bazel-* ignore; repair stale paths explicitly before syncing", s.name)
		}
	}
	return true, nil
}
