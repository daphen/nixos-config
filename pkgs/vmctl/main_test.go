...
func TestRealLifecycleCreatesSeedsAlignsAndReusesCanonicalMirror(t *testing.T) {
	...
	listing := fmt.Sprintf(`[{"name":"vmwt-every-3064","alpha":{"protocol":"ssh","user":"david_karlsson_lovable_dev","host":"dev-heidr-2a39.workstation.lovable.net","path":"/home/david_karlsson_lovable_dev/src/lovable-every-3064"},"beta":{"protocol":"local","path":%q}}]`, local)
	...
	for name, script := range map[string]string{"ssh": ssh, "mutagen": mutagen, "rsync": rsync} {
...
func TestAlignCleanVMAdvanceRefreshesIndexWithRealGit(t *testing.T) {
	...
	for name, script := range map[string]string{"mutagen": mutagen, "wt": "#!/bin/sh\nexit 99\n", "rsync": "#!/bin/sh\nexit 99\n"} {
		if err := os.WriteFile(filepath.Join(bin, name), []byte(script), 0o755); err != nil {
			t.Fatal(err)
		}
	}
...
case "$name" in
 ...
 git)
   case "$*" in
     ...
     *"rev-parse --show-toplevel"*) while IFS= read -r root; do echo "$root"; done < "$HOME/wt-path" ;;
 ...
 mutagen)
   case "$*" in
     "sync list --template "*)
       [ "${MUTAGEN_MODE-}" = matching ] && printf '[{"name":"vmwt-every-3315","alpha":{"protocol":"ssh","user":"david_karlsson_lovable_dev","host":"dev-heidr-2a39.workstation.lovable.net","path":"/home/david_karlsson_lovable_dev/src/lovable-every-3315"},"beta":{"protocol":"local","path":"%s/work/lovable.daphen-every-3315"}}]\n' "$HOME" && exit 0
       ...
       [ "${MUTAGEN_MODE-}" = empty-success ] && echo '[]' && exit 0
       exit 1 ;;
...
func matchingSessionJSON(home string) string {
	return fmt.Sprintf(`[{"name":"vmwt-every-3315","alpha":{"protocol":"ssh","user":"david_karlsson_lovable_dev","host":"dev-heidr-2a39.workstation.lovable.net","path":"/home/david_karlsson_lovable_dev/src/lovable-every-3315"},"beta":{"protocol":"local","path":%q}}]`, filepath.Join(home, "work/lovable.daphen-every-3315"))
}
...
func fixture(t *testing.T, body string) (string, string, string) {
	...
	for _, name := range []string{"git", "ssh", "mutagen", "wt", "rsync", "direnv"} {
