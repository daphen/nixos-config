...
The Deck keeps SteamOS, boots Hyprland by default, and switches to a non-nested
Gamescope session through the **Gaming Mode** desktop entry. Its committed
hardware file names only the new `NIXOS_DECK_ROOT` and `NIXOS_DECK_EFI` labels;
they are placeholders until the real partition table is inspected.
...
1. In Hyprland, certify orientation/touch, all canvas keyboard and Deck
   ...
   Bluetooth, and dock hotplug. The session wrapper releases InputPlumber's
   desktop profile before **Gaming Mode** starts.
1. In Gaming Mode, test a native and Proton title, then use Steam's Desktop Mode
   action to return to Hyprland. Finally boot the previous NixOS generation and
   Valve recovery media once.
