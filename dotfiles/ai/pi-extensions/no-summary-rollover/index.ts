...
const USER_BYTES = 8_000;
const OUTCOME_BYTES = 5_000;
...
export default function noSummaryRollover(pi: ExtensionAPI) {
  pi.on("session_before_compact", (event) => {
    if (event.reason !== "threshold" || event.preparation.isSplitTurn) return;
    if (event.preparation.messagesToSummarize.length === 0 && !event.preparation.previousSummary) return;
    ...
    return {
      compaction: {
        ...
        details: { strategy: "deterministic-threshold-v1", readFiles: files.readFiles, modifiedFiles: files.modifiedFiles },
      },
    };
  });
}
