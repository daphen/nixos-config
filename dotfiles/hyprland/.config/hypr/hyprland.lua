...
geometry = function(ctx, row, column, id)
	...
	local y = ctx.area.y + GAP - state.camera.y - (state.keyboard_offset or 0) + (row - 1) * (ctx.area.h - GAP)
...
if DECK_MODE then
	...
	hl.bind("F7", function() deck_rt_down = true end, { ignore_mods = true })
	...
	local function rt_face_key(index)
		if not deck_rt_down then return false end
		local window = hl.get_active_window()
	...
	hl.bind("Escape", function()
		...
		if rt_face_key(2) then return end
...
if REAL_MODE and DECK_MODE then
	local function app(key, selector, command)
		hl.bind("SUPER + " .. key, hl.dsp.exec_cmd(string.format("%sjump-or-exec %q %q", SCRIPTS, selector, command)))
	end
