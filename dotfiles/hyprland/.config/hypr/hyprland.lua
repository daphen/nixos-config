...
if DECK_MODE then
    ...
    local radial_open, radial_outer = false, false
    ...
    local function radial_direction()
        local x = (radial_held[4] and 1 or 0) - (radial_held[1] and 1 or 0)
        local y = (radial_held[2] and 1 or 0) - (radial_held[3] and 1 or 0)
    ...
    local function radial_press(index)
        ...
        local direction = radial_direction()
        if radial_open then radial_ipc("direction", direction) return end
        local window = hl.get_active_window()
    ...
    hl.on("input.keyboard.key", function(keycode, _, key_state)
        if keycode >= 67 and keycode <= 70 and key_state == 0 then
            ...
            local direction = radial_direction()
            if radial_open and direction then radial_ipc("direction", direction) end
    ...
    for key, direction in pairs(deck_directions) do
        hl.bind(key, function()
            if radial_open then
                if key == "F13" then radial_ipc("page", -1)
                elseif key == "F16" then radial_ipc("page", 1) end
                return
            end
        ...
        hl.bind("SUPER + " .. key, function()
            if radial_open then
                if key == "F13" then radial_ipc("page", -1)
                elseif key == "F16" then radial_ipc("page", 1) end
            else hl.dispatch(hl.dsp.layout("focus " .. direction)) end
        end, { repeating = true })
    ...
    hl.bind("F20", hl.dsp.window.close())
    ...
    hl.bind("F22", function()
        local command = "deck-osk-toggle"
        for _, layer in ipairs(hl.get_layers()) do
            if layer.mapped and layer.keyboard_interactivity == 1 then
    ...
    hl.bind("SUPER + F22", hl.dsp.layout("resize h"), { repeating = true })
    hl.bind("SUPER + SPACE", hl.dsp.exec_cmd("qs ipc call -- launcher toggle"))
    hl.bind("SUPER + RETURN", function()
        if radial_open then radial_ipc("activate", 0)
...
local deck_gesture_mode = false
local function finish_pan()
...
if DECK_MODE then
    hl.bind("F17", function() deck_gesture_mode = true end, { ignore_mods = true })
    hl.bind("F17", function()
        deck_gesture_mode = false
        finish_pan()
...
hl.gesture({
    ...
    action = {
        start = function()
            if DECK_MODE and not deck_gesture_mode then return end
            hl.dispatch(hl.dsp.layout("pan-begin"))
            if not overview_timer then
        ...
        finish = function()
            if not DECK_MODE then finish_pan() end
        end,
    },
})
...
hl.on("hyprland.start", function()
    if DECK_MODE then
        local startup = {
            ...
            "wvkbd-mobintl -H 280 -L 280 --auto --hidden",
            "steam -silent",
        }
