...
# because Hyprland-spawned processes don't see home.sessionVariables until
# re-login; every launcher sources this file.
export GTK_USE_PORTAL=1

# Wayland app-id assigned via --class. KEPT STABLE across browser
# switches (it's just a label) so Hyprland window-rules and ws-focus's
...
BROWSER_FLAGS=(
    ...
    # Run natively on Wayland (Hyprland) instead of XWayland. Without this
    # Chromium defaults to X11 ozone, so getDisplayMedia uses the X11
    # capturer — which has no real desktop to grab under Hyprland, breaking
...
# color-scheme on each launch. Needed until Hyprland itself inherits this
