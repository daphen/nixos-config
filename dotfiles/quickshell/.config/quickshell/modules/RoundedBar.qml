...
PanelWindow {
    ...
    readonly property string metricTooltipText: metricTooltipTarget === weatherMetric
        ...
        : metricTooltipTarget === networkMetric
            ? (networkMetric.kind === "eth" ? "100%"
                : networkMetric.kind === "wifi" ? networkMetric.strength + "%" : "0%")
        : metricTooltipTarget === audioMetric
            ? Math.round((audioMetric.muted ? 0 : audioMetric.volume) * 100) + "%"
        : metricTooltipTarget === batteryMetric ? Math.round(batteryMetric.percentage) + "%"
        : ""
