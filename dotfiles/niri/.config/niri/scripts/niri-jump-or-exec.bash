...
# --here: bring the window to the CURRENT workspace instead of teleporting you to
# its workspace. For single-window companions (chat, mail) you want the app to come
# to you; plain focus-window yanked you off whatever you were working on.
HERE=false
if [ "${1:-}" = "--here" ]; then
    HERE=true
    shift
...
# Check if matching by title or regex
...
try:
    ...
    # Sort matches by focus_timestamp descending — most recent first.
    # That way the cycle order is predictable, and the fallback lands on
    # a recently-used match instead of whatever niri happens to return.
    def ts(w):
...
if [ "$RESULT" = "NONE" ]; then
    ...
    # Window exists. With --here, pull it onto the workspace you're looking at
    # before focusing, so the app joins you instead of dragging you elsewhere.
    if [ "$HERE" = true ]; then
        # A workspace REFERENCE is a name or an index — a bare number is read as
        # the index, never the id, so prefer the name and fall back to idx.
        WS=$(niri msg --json workspaces | python3 -c "
import sys, json
for w in json.load(sys.stdin):
    if w.get('is_focused'):
        print(w.get('name') or w.get('idx') or '')
        break
")
        if [ -n "$WS" ]; then
            niri msg action move-window-to-workspace --window-id "$RESULT" --focus false "$WS" >/dev/null 2>&1 || true
        fi
    fi
