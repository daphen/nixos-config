...
let parent=null,frame=null,specimen=null,liveId=null,specimenId=null,before=null,after=null,restored=null,result=null,failure=null,mutationSent=false;
try{
 ...
 let hydrated=false,apiStatus=null,apiAuthorization=null,startResponses=[],blockedStarts=0,replayedStarts=0,requestMethods=new Map(),requestAuthorizations=new Map();
 while(Date.now()<deadline){
  for(const event of parent.events.splice(0)){if(event.method==="Fetch.requestPaused"){if(event.params.request.method==="POST"&&event.params.request.url.includes("/sandbox/start")&&!allowStart){const authorization=event.params.request.headers?.Authorization??event.params.request.headers?.authorization;if(prior?.state==="ready"&&authorization){const discovery=await fetch(`http://127.0.0.1:__API_PORT__/projects/${projectId}/sandbox/url`,{headers:{Authorization:authorization},signal:AbortSignal.timeout(5000)});const discovered=discovery.ok?await discovery.json():null;if(discovered?.url&&new URL(discovered.url).hostname===`${projectId}.lovableproject-dev.com`){apiAuthorization=authorization;apiStatus=discovery.status;replayedStarts++;await parent.send("Fetch.continueRequest",{requestId:event.params.requestId,url:`http://127.0.0.1:__API_PORT__/projects/${projectId}/sandbox/url`,method:"GET"});}else{blockedStarts++;await parent.send("Fetch.failRequest",{requestId:event.params.requestId,errorReason:"BlockedByClient"});}}else{blockedStarts++;await parent.send("Fetch.failRequest",{requestId:event.params.requestId,errorReason:"BlockedByClient"});}}else await parent.send("Fetch.continueRequest",{requestId:event.params.requestId});}if(event.method==="Network.requestWillBeSent"){requestMethods.set(event.params.requestId,event.params.request.method);const authorization=event.params.request.headers?.Authorization??event.params.request.headers?.authorization;if(authorization)requestAuthorizations.set(event.params.requestId,authorization);}if(event.method==="Network.responseReceived"){const response=event.params.response,parsed=new URL(response.url);if(parsed.pathname.endsWith(`/projects/${projectId}`)&&[String(__API_PORT__),String(__COMPAT_PORT__)].includes(parsed.port)&&response.status>=200&&response.status<300){apiStatus=response.status;apiAuthorization=requestAuthorizations.get(event.params.requestId)??apiAuthorization;}if(response.url.includes(`/projects/${projectId}/sandbox/start`)&&requestMethods.get(event.params.requestId)==="POST")startResponses.push({status:response.status,requestId:event.params.requestId,replayed:replayedStarts>0});}}
 ...
 if(startResponses.length>1)throw Error(`observed ${startResponses.length} sandbox-start requests`);
 if(startResponses.length===1&&startResponses[0].status!==200)throw Error(`sandbox-start returned ${startResponses[0].status}`);
...
'''
    ...
    def render_units(self) -> dict[str, str]:
        ...
        browser = f"""[Unit]\nDescription=Agent review {self.context_id} isolated authenticated browser\nRequires={self.units['keyless_proxy']}\nAfter={self.units['keyless_proxy']}\n\n[Service]\nType=simple\nExecStart=/run/current-system/sw/bin/chromium --headless=new --disable-gpu --ozone-platform=headless --user-data-dir={self.root}/browser-profile --remote-debugging-address=127.0.0.1 --remote-debugging-port={p.browser_debug} --disable-extensions --no-first-run --no-default-browser-check about:blank\nRestart=on-failure\nRestartSec=2\nStandardOutput=append:{self.root}/browser.log\nStandardError=append:{self.root}/browser.log\n"""
        return {
