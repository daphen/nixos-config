...
class ManualHarness:
    ...
    def context(self) -> HarnessContext:
        ...
        return HarnessContext(
            ...
            ports=self.ports,
            units=self.units,
            remote_units={"vm_slice": f"agent-review-{self.context_id}-vm.service"},
            readiness=readiness,
            teardown=teardown,
        )
...
if(!live&&allowStart&&prior)throw Error("sandbox-start one-shot marker already exists; refusing a retry");
if(!live&&allowStart)fs.writeFileSync(certification,JSON.stringify({state:"attempting",at:new Date().toISOString()})+"\n",{flag:"wx"});
const snapshotExpression=`(()=>{const target=document.querySelector("button")??document.body?.firstElementChild??document.body;if(!target)return "";const style=getComputedStyle(target);return JSON.stringify({html:target.outerHTML,style:{backgroundColor:style.backgroundColor,color:style.color,cursor:style.cursor,display:style.display,opacity:style.opacity,padding:style.padding,pointerEvents:style.pointerEvents}})})()`;
let parent=null,frame=null,specimen=null,liveId=null,specimenId=null,before=null,after=null,restored=null,result=null,failure=null,mutationSent=false,typedControls=null;
try{
 parent=new CDP(page.webSocketDebuggerUrl);await parent.open();await parent.send("Network.enable");await parent.send("Runtime.enable");await parent.send("Page.enable");await parent.send("Fetch.enable",{patterns:[{urlPattern:"*sandbox/start*",requestStage:"Request"}]});
 const capture=`window.__agentReviewMessages=[];window.addEventListener("message",event=>{if(event.data?.type==="CONNECTION_HELLO")window.__agentReviewMessages.push(event.data)})`;
 await parent.send("Page.addScriptToEvaluateOnNewDocument",{source:capture});await parent.evaluate(capture);
 if(!live||!page.url.includes(`/projects/${projectId}`))await parent.send("Page.navigate",{url:browserUrl});else await parent.send("Page.reload",{ignoreCache:true});
 let hydrated=false,apiStatus=null,apiAuthorization=null,startRequest=null,startResponse=null,blockedStarts=0,blockedDuplicateStarts=0,replayedStarts=0,requestMethods=new Map(),requestAuthorizations=new Map();
 while(Date.now()<deadline){
  for(const event of parent.events.splice(0)){if(event.method==="Fetch.requestPaused"){const request=event.params.request,isSandboxStart=request.method==="POST"&&request.url.includes("/sandbox/start");if(isSandboxStart&&!allowStart){const authorization=request.headers?.Authorization??request.headers?.authorization;if(prior?.state==="ready"&&authorization){const discovery=await fetch(`http://127.0.0.1:__API_PORT__/projects/${projectId}/sandbox/url`,{headers:{Authorization:authorization},signal:AbortSignal.timeout(5000)});const discovered=discovery.ok?await discovery.json():null;if(discovered?.url&&new URL(discovered.url).hostname===`${projectId}.lovableproject-dev.com`){apiAuthorization=authorization;apiStatus=discovery.status;replayedStarts++;await parent.send("Fetch.continueRequest",{requestId:event.params.requestId,url:`http://127.0.0.1:__API_PORT__/projects/${projectId}/sandbox/url`,method:"GET"});}else{blockedStarts++;await parent.send("Fetch.failRequest",{requestId:event.params.requestId,errorReason:"BlockedByClient"});}}else{blockedStarts++;await parent.send("Fetch.failRequest",{requestId:event.params.requestId,errorReason:"BlockedByClient"});}}else if(isSandboxStart){if(startRequest){blockedDuplicateStarts++;await parent.send("Fetch.failRequest",{requestId:event.params.requestId,errorReason:"BlockedByClient"});}else{startRequest={fetchRequestId:event.params.requestId,method:request.method,url:request.url,observedAt:new Date().toISOString()};await parent.send("Fetch.continueRequest",{requestId:event.params.requestId});}}else await parent.send("Fetch.continueRequest",{requestId:event.params.requestId});}if(event.method==="Network.requestWillBeSent"){requestMethods.set(event.params.requestId,event.params.request.method);const authorization=event.params.request.headers?.Authorization??event.params.request.headers?.authorization;if(authorization)requestAuthorizations.set(event.params.requestId,authorization);if(startRequest&&!startRequest.networkRequestId&&event.params.request.method==="POST"&&event.params.request.url.includes("/sandbox/start"))startRequest.networkRequestId=event.params.requestId;}if(event.method==="Network.responseReceived"){const response=event.params.response,parsed=new URL(response.url);if(parsed.pathname.endsWith(`/projects/${projectId}`)&&[String(__API_PORT__),String(__COMPAT_PORT__)].includes(parsed.port)&&response.status>=200&&response.status<300){apiStatus=response.status;apiAuthorization=requestAuthorizations.get(event.params.requestId)??apiAuthorization;}if(!startResponse&&response.url.includes(`/projects/${projectId}/sandbox/start`)&&requestMethods.get(event.params.requestId)==="POST")startResponse={status:response.status,requestId:event.params.requestId,observedAt:new Date().toISOString()};}}
 ...
 if(!hydrated)throw Error("authenticated project UI did not hydrate within 60s");
 ...
 const canonicalSelection=await parent.evaluate(`(()=>{const host=document.querySelector('.tl-container')??document.querySelector('#root'),key=Object.keys(host??{}).find(name=>name.startsWith('__reactFiber$'));let fiber=host?.[key];while(fiber?.return)fiber=fiber.return;const queue=[fiber];let editor=null;while(queue.length&&!editor){const node=queue.shift();if(!node)continue;let context=node.dependencies?.firstContext;while(context){const value=context.memoizedValue;if(value&&typeof value.getShape==='function'&&typeof value.select==='function'){editor=value;break}context=context.next}queue.push(node.child,node.sibling)}if(!editor)throw Error('tldraw editor missing');const shape=editor.getShape('shape:component-anchor-Button');if(!shape||shape.props?.kind!=='component'||shape.meta?.dsComponentKey!=='Button')throw Error('canonical Button shape missing');editor.select(shape.id);return {id:shape.id,kind:shape.props.kind,componentKey:shape.meta.dsComponentKey}})()`);
 ...
 liveId=live.id;frame=new CDP(live.webSocketDebuggerUrl);await frame.open();await frame.send("Runtime.enable");
 await parent.evaluate(`window.__agentReviewMessages=window.__agentReviewMessages??[]`);
 list=await targets();const specimenTarget=list.find(target=>target.type==="iframe"&&target.url.includes(".lovableproject-dev.com/__component/preview/"));if(!specimenTarget)throw Error("live specimen iframe target missing");
 ...
 await specimen.evaluate("location.reload()");
 ...
 while(Date.now()<deadline){
  ...
  try{hello=await parent.evaluate(`window.__agentReviewMessages.find(message=>message.type==="CONNECTION_HELLO")??null`);}catch{}
  if(websocket&&hello)break;await sleep(100);
 }
 if(!websocket)throw Error("live iframe-owned Vite WebSocket was not connected");
 if(!hello?.payload||typeof hello.payload.protocolVersion!=="number"||!hello.payload.sessionId||!Array.isArray(hello.payload.capabilities)||!hello.payload.scriptVersion)throw Error("CONNECTION_HELLO runtime fingerprint is incomplete");
 const runtimeSource=await parent.evaluate(`fetch('http://127.0.0.1:__API_PORT__/v1/git/files/mockupPreviewPlugin.ts?project_id=${projectId}',{headers:{Authorization:${JSON.stringify(apiAuthorization)}}}).then(async response=>{if(!response.ok)throw Error('runtime source returned '+response.status);return response.text()})`);
...
'''
    ...
    def render_units(self) -> dict[str, str]:
        ...
        browser = f"""[Unit]\nDescription=Agent review {self.context_id} isolated authenticated browser\nRequires={self.units['keyless_proxy']}\nAfter={self.units['keyless_proxy']}\n\n[Service]\nType=simple\nExecStart=/run/current-system/sw/bin/chromium --headless=new --disable-gpu --ozone-platform=headless --window-size=1920,1080 --user-data-dir={self.root}/browser-profile --remote-debugging-address=127.0.0.1 --remote-debugging-port={p.browser_debug} --disable-extensions --no-first-run --no-default-browser-check about:blank\nRestart=on-failure\nRestartSec=2\nStandardOutput=append:{self.root}/browser.log\nStandardError=append:{self.root}/browser.log\n"""
        return {
...
'''
    ...
    def start(self) -> None:
        ...
        for name in (self.units["connectivity"], self.units["keyless_proxy"], self.units["browser"]):
            link = unit_dir / name
        ...
        run(["systemctl", "--user", "start", self.units["connectivity"], self.units["keyless_proxy"], self.units["browser"]], timeout=20)
