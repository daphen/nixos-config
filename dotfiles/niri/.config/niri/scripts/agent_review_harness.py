...
const isSelectedLive=target=>target.type==="iframe"&&new URL(target.url).pathname==="/"&&isProjectHost(new URL(target.url).hostname);
...
try{
 ...
 await parent.send("Page.navigate",{url:`${browserUrl}&agent_review_certification=${Date.now()}`});
 ...
 list=await targets();const specimenTarget=list.find(target=>target.type==="iframe"&&new URL(target.url).pathname.startsWith("/__component/preview/")&&isProjectHost(new URL(target.url).hostname));if(!specimenTarget)throw Error("live specimen iframe target missing");
